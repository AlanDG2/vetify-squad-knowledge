# Prompt para devs — dejar el README a nivel de contexto para QA + IA (versión general, cualquier repo)
...
> vez de tener que reconstruirlo leyendo archivo por archivo vía API.
>
...
> Copilot Chat, etc.) parado en la raíz de TU repo — sirve para cualquier repo del squad, no uno
> en particular. Si no usás un asistente de IA, sirve como checklist para hacerlo a mano. Es un
> pedido puntual (una vez por repo, o cuando el README quedó viejo) — no un proceso nuevo por cada
> PR.
...
Nivel de detalle esperado: una tabla compacta con el contrato HTTP (ruta, método, rol requerido),
una tabla con los códigos de error de negocio (código, HTTP, causa, qué revisar), una tabla con
las variables de entorno reales, y una sección corta de seguridad/deploy. Tablas, no párrafos
narrativos largos.

Reglas:
- NUNCA pegar secretos, tokens, API keys, contraseñas ni JWTs reales — ni siquiera "de ambiente
  bajo". Usar placeholders (`<tu-api-key>`).
