...
5. La **validación de ARCA (automática) confirmada que NO funciona en ambientes bajos/QA** (siempre falla) — existe **`IMAS-4472`** ("[Reintegros] Validación de factura con ARCA no funciona en QA y Producción", padre, "In Validation"). **Actualización 2026-09-02**: se deployó un fix (`IMAS-4533`, subtarea Desarrollo, "Hecho") que agregó un workaround manual, pero **la validación automática en sí sigue devolviendo HTTP 400 en QA** — el ticket pedía que ARCA respondiera bien en QA y eso no se cumple (`BUG-031`, subtarea QA `IMAS-4534` en Backlog). No confiar en que "Hecho" en la subtarea de desarrollo signifique que el problema de fondo está resuelto.
...
**Engage es el sistema central único de afiliados, no una pieza más** (aclaración de arquitectura, llamada 2026-09-14 con Pablo Mendoza, equipo Soluciones): "usuario" (login/Auth0) y "afiliado" (cliente que compró) son sistemas distintos — **todo lo que se vende en cualquier producto/canal (Vetify, OSDE, Flux, Salesforce, landing) termina en Engage**. Ike hoy no tiene una base propia de clientes que lo alimente (en construcción, sin fecha). Para operar afiliados directamente en QA (ver/buscar por DNI/borrar/modificar) hace falta un usuario con permisos de Engage — lo administra el equipo de **Soluciones** (responsable real: **Leonel**).
...
- **Banner de beneficios en Home — "Vetify PLUS" / "Cooper" (IMAS-4356, verificado en vivo 2026-08-26/27)**: la pantalla de Home tiene un banner de "descuentos y beneficios" (también replicado como entrada propia en el menú lateral, sección "Cuenta") que **varía según el segmento y el estado del perfil del usuario**, no es fijo:
  ...
  - **⚠️ Lo de arriba quedó parcialmente desactualizado por `IMAS-4435`** ("Implementación de Banner Cooper con mayor protagonismo", seguimiento directo de `IMAS-4356` para Vetify B2C). **Confirmado en vivo 2026-08-31**: el banner de Home ahora sí muestra Cooper para B2C también ("Beneficio exclusivo clientes Vetify", botón "Ir a Cooper") — ya no es solo Vetify PLUS. En el menú lateral, "Cuenta" ya no tiene el ítem de beneficios: existe una **sección nueva dedicada "Beneficios"**.
  - **Regla de negocio real (confirmada por el dev, 2026-08-31)**: (1) todos los usuarios ven el banner Cooper en Home; (2) todos excepto OSDE ven **Cooper y Vetify PLUS** en el menú; (3) los de OSDE ven **solo Cooper** en el menú (sin Vetify PLUS).
  - **Estado real por segmento (2026-09-08)**: banner en Home — cumple en los 3 segmentos (B2C, OSDE Capitado, OSDE Adquirente). Menú "Beneficios" — cumple en B2C y OSDE Capitado; **OSDE Adquirente sigue mostrando Cooper + Vetify PLUS juntos, no cumple la regla (3)** — bug conocido, no reportado a Jira todavía.
  - **Gotcha de metodología (aplica a cualquier comparación entre cuentas/segmentos)**: el Service Worker (Workbox) cachea respuestas de `/api/*` con una clave que **no varía por usuario** — comparar 2 cuentas con logins sucesivos en la misma pestaña sin limpiar Service Worker + caches entre cada login sirve datos de la cuenta anterior a la nueva, dando falsos resultados de segmentación.
  - **Menú lateral completo confirmado (`SideMenuSection.ts`), 15 ítems**: Inicio, Mascotas, Perfil, Videollamadas, Veterinarias, Reintegros, Planes y coberturas, Vetify Plus, Emergencias, Asistencia presencial, Asistencia a domicilio, Historial de atención, Ayuda, Cooper, Facturas, más "Cerrar sesión". **No existe ningún ítem "Atención de Red"** — si el roadmap de la PO lo menciona, es mislabeling, no una funcionalidad real sin mapear.
...
- **Videollamadas** (`videocall/`): agendar (`VideocallFormPage` + `CalendarSchedulingComponent`), reprogramar (`RescheduleVideocallPage`), cancelar (`CancelVideocallModal`), visualizar (`VideocallViewPage`) — consulta veterinaria por videollamada.
  ...
  - **Límite de videollamadas por plan — YA IMPLEMENTADO y probado (`IMAS-4546`, confirmado en vivo 2026-09-18/19)**: lo que en la review del 2026-08-21 estaba "recién a definir" ya está en producción-QA. Plan "Esencial" de OSDE Capitado (y también Flux Capitado Esencial — la solución es parametrizable cross-producto, no hardcodeada a un solo plan) tiene **límite de 2 videollamadas por año calendario**; el resto de los planes (ej. Emergencias/Cachorro B2C) siguen ilimitados. Reglas confirmadas:
    - En la pantalla de Motivo, la mascota con plan limitado muestra **"Cupo disponible: X de Y"**; con plan ilimitado no se muestra ningún contador (endpoint real: `GET /api/services/pets/pet/{petId}/videollamada/cobertura` → `{disponible, limite}`, `limite:100` es el valor sentinela de "sin límite").
    - El cupo se descuenta **al agendar el turno, no al consumirlo** — un turno agendado y luego no usado igual cuenta.
    - Cancelar con **≥30 min de anticipación restituye el cupo**; cancelar con **<30 min lo pierde** (mismo corte de tiempo que la regla general de cancelación, pero acá además define si el cupo anual se recupera o no).
    - Al agotar el cupo hay **2 modales de bloqueo distintos** según la causa: el histórico de `IMAS-3909` ("Superaste el límite de videollamadas por mascota") si además hay 2 turnos concurrentemente agendados, o uno nuevo de esta HU ("Alcanzaste el límite de X videollamadas anuales / Para agendar una nueva, tendrás que esperar que se renueve tu cupo") si el cupo se agota con menos de 2 turnos concurrentes.
    - En cuentas multi-mascota, cada mascota tiene su cupo completamente independiente (agotar el de una no afecta a la otra) — el selector "Seleccioná tu mascota" muestra "Cupo ilimitado" o "Cupo disponible: X de Y" por mascota.
    - **Gap de cobertura sin cerrar, no es AC formal de la HU**: existe un 5to "caso especial" (título de las subtareas técnicas `IMAS-4712`/`IMAS-4713`) para un plan que directamente NO incluye videollamada — no se encontró ningún producto/plan real en QA que dispare ese estado; pregunta abierta para el equipo de dev.
    - Automatizado en `tests/projects/vetify-webapp/videocall.spec.ts` (suite `TS-06`), ver `automation/docs/user-stories/IMAS-4546-limitar-videollamadas.tests.md` para el detalle completo.
  ...
  - **Edición desde la confirmación (`IMAS-4310`, confirmado en vivo 2026-09-17)**: la pantalla "Revisá los datos y confirmá tu turno" ahora tiene botón de editar en los 3 campos (Fecha y hora, Motivo, Adjuntos) — antes solo lo tenía Fecha y hora. Los 3 cambios se pueden combinar sin pisarse entre sí. Bug menor sin reportar: al reabrir el selector de horario desde ahí, tocar la franja "Noche" no cambia la lista de horarios mostrada (sigue en "Tarde").
...
**⚠️ `{{QUANTUM_BASE_URL}}` migró de dominio (confirmado 2026-09-14, tras casi 2 semanas de 404 intermitente)**: catálogo y pago (`payment/*`, `catalog/*` de la tabla de arriba) hoy viven en **`https://qa-quantum.ike.ar/api/v1/jengage/...`** — un host **completamente distinto** de `qa.vetify.com.ar`, que sigue sirviendo solo para el login (`/api/quantum/jauth/token`) y Salesforce (`/api/sf/first-step`/`second-step`). El mismo token de login sirve en los 2 dominios. Este path caído es propiedad del **equipo COP**, no del equipo que atiende Vetify (Oscar Tello confirmó "todo lo que es engage lo administra la gente de COP, nosotros no tenemos control sobre esas APIs") — si vuelve a caerse, escalar ahí, no al equipo Vetify. En `automation`, la variable de entorno es `VETIFY_QUANTUM_BASE_URL`.
